  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.id) {
      try {
        ...
        if (session.user.id !== decodedId || !validateUserAccess(encodedUserId, userRole, "admin")) {
          setRedirected(true);
          router.replace("/auth/signin");
        }
      } catch {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="p-8">
        ...
        <div className="grid gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <h2 className="text-xl font-semibold mb-4">Tour Management</h2>
            <p className="text-gray-600">Manage tours, operators, and bookings</p>
          </div>
        </div>
      </div>
    </div>
