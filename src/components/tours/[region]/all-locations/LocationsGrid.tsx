  ...
  return (
    <section>
      <div style={{
        ...
        {locations.map((loc) => (
          <article key={loc.location} style={{ border: "1px solid #e6e6e6", borderRadius: 8, overflow: "hidden" }}>
            <Link href={`/tours/${encodeURIComponent(region)}/locations/${encodeURIComponent(loc.location)}`}>
              <a style={{ display: "block", textDecoration: "none", color: "inherit" }}>
                <div style={{ height: 140, background: "#f6f6f6", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  {loc.sampleImage ? (
                    <img src={loc.sampleImage} alt={loc.location} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                  ) : (
                    <div style={{ padding: 12 }}>{loc.location}</div>
                  )}
                </div>
                <div style={{ padding: 12 }}>
                  <h3 style={{ margin: "0 0 8px 0", fontSize: 16 }}>{loc.location}</h3>
                  <div style={{ color: "#666", fontSize: 14 }}>{loc.count} tour{loc.count > 1 ? "s" : ""}</div>
                </div>
              </a>
            </Link>
          </article>
