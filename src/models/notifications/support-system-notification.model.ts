...
import {
  SUPPORT_SYSTEM_NOTIFICATION_PRIORITY,
  SUPPORT_SYSTEM_NOTIFICATION_TYPE,
  SupportSystemNotificationPriority,
  SupportSystemNotificationType,
} from "@/constants/notifications/support-system-notification.const";
...
export interface ISupportSystemNotification extends Document {
  type: SupportSystemNotificationType;
  title: string;
  message: string;
  link?: string;
  icon?: string;
  ...
  priority: SupportSystemNotificationPriority;
...
const SupportSystemNotificationSchema = new Schema<ISupportSystemNotification>(
  {
    type: {
      type: String,
      enum: Object.values(SUPPORT_SYSTEM_NOTIFICATION_TYPE),
      required: true,
      index: true,
    },
    title: {
    ...
    priority: {
      type: String,
      enum: Object.values(SUPPORT_SYSTEM_NOTIFICATION_PRIORITY),
      default: SUPPORT_SYSTEM_NOTIFICATION_PRIORITY.MEDIUM,
      index: true,
    },
...
SupportSystemNotificationSchema.pre("save", function (next) {
  if (
    this.priority === SUPPORT_SYSTEM_NOTIFICATION_PRIORITY.LOW ||
    this.priority === SUPPORT_SYSTEM_NOTIFICATION_PRIORITY.MEDIUM
  ) {
